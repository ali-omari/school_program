...
class add_student(QWidget):
    ...
    def save_newstudent(self):
        try:
            ...
            idd=self.line_num.get_txt_editline()
            namee=self.line_name.get_txt_editline()
            CLasss=self.combo_student.get_txtcombo()
            bornn=self.line_born.get_txt_editline()
            phonee=self.line_phone.get_txt_editline()

            mydatabase.insert_student(idd,namee,CLasss,bornn,phonee)
    ...
    def button(self):
        btn_save_student=tool1.make_btn('حفظ',self,20,235,100,35,15,'green',self.save_newstudent)
